...
//   }
//
// 1. and 2. are the same, the important thing is that the dynamic key can also describe every static key,
...
// The best solution for now is 4, where we create an union of all the possible types for the dynamic keys.
// The only drawback is that we might allow some type that won't work in the dynamic keys.
// Furthermore, option 4 must take into account the types of the extended class properties (if present).
...
function serializeAdditionalPropertiesType (type: M.Interface): string {
  ...
  const openGenerics = type.generics?.map(t => t.name) ?? []
  let code = `export interface ${createName(type.name)}${buildGenerics(type.generics)}${buildInherits(type)} {\n`
  ...
  if (Array.isArray(dictionaryOf.generics) && dictionaryOf.generics[1].kind === 'user_defined_value') {
    ...
    if (Array.isArray(dictionaryOf.generics)) {
      const dynamicTypes = [...new Set([buildValue(dictionaryOf.generics[1], openGenerics)].concat(types).concat(extendedPropertyTypes))]
      code += `  [property: string]: ${dynamicTypes.join(' | ')}\n`
    }
  }
  code += '}\n'
  return code
