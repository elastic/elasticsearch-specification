...
function buildRequest(type) {
    ...
    for (const property of type.path) {
        code += `  ${cleanPropertyName((_c = property.codegenName) !== null && _c !== void 0 ? _c : property.name)}${property.required ? '' : '?'}: ${buildValue(property.type, openGenerics)}\n`;
    }
    const pathPropertiesNames = type.path.map(property => { var _a; return (_a = property.codegenName) !== null && _a !== void 0 ? _a : property.name; });
    for (const property of type.query) {
        if (pathPropertiesNames.includes(property.name))
            continue;
        code += `  ${cleanPropertyName((_d = property.codegenName) !== null && _d !== void 0 ? _d : property.name)}${property.required ? '' : '?'}: ${buildValue(property.type, openGenerics)}\n`;
    }
