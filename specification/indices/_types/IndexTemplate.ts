...
/**
 * This is the same as IndexTemplateSummary and must be kept in sync with it,
 * the only difference being that "lifecycle" supports the "rollover" field
 * in this case (when used as part of get index template response)
 */
export class IndexTemplateSummaryRes {
  /**
   * Aliases to add.
   * If the index template includes a `data_stream` object, these are data stream aliases.
   * Otherwise, these are index aliases.
...
/**
 * This is the same as IndexTemplate and must be kept in sync with it,
 * the only difference being that "template.lifecycle" supports the "rollover" field
 * in this case (when used as part of get index template response)
 */
export class IndexTemplateRes {
  /**
   * Array of wildcard (`*`) expressions used to match the names of data streams and indices during creation.
   */
  index_patterns: Names
   ...
   * It may optionally include an `aliases`, `mappings`, or `settings` configuration.
   */
  template?: IndexTemplateSummaryRes
  /**
   * Version number used to manage index templates externally.
   * This number is not automatically generated by Elasticsearch.
   */
