...
export class RequestChatCompletion {
   ...
   * This is used to control the model's reasoning process during generation.
   * Could be used to specify either the level of effort the model should put into reasoning,
   * the max number of tokens the model can use for reasoning,
   * or whether reasoning should be enabled with default settings.
   * Also includes setting to control the level of detail included in the summary returned in the response,
   * and a setting to control whether reasoning details should be returned in the response at all.
   * Example (effort):
   * ```
   * {
   *    "reasoning": {
   ...
   * }
   * ```
   * Currently supported only for OpenRouter provider.
   */
  reasoning?: Reasoning
  /**
