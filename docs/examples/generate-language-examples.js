...
async function generateLanguages(example) {
  ...
  // Java examples are generated by an out-of-process jar (the java-request-converter),
  // driven here via java-caller. It runs whenever the jar path is provided through
  // JAVA_ES_REQUEST_CONVERTER_JAR, so a single `generate-language-examples` target
  // covers every language; when the variable is unset (e.g. local runs without the
  // jar) Java is simply skipped.
  if (process.env.JAVA_ES_REQUEST_CONVERTER_JAR) {
